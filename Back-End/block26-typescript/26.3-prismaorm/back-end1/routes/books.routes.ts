...
import validationBook from '../middleware/validationBook';
...
router.post('/books',validationBook,async (req: Request, res: Response): Promise<Response> => {
...
router.put('/books/:isbn',async (req: Request, res: Response): Promise<Response> => {
  const { isbn } = req.params;
  ...
  if (index === -1) return res.status(StatusCode.NOT_FOUND).json(NotFoundMessage);
