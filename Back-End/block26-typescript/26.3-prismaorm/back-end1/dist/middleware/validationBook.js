...
function validationBook(req, res, next) {
    ...
    if (!valid)
        return res.status(StatusCode_1.default.BAD_REQUEST).json(`O campo ${property} é obrigatoório`);
    [valid, property] = validateValues(book);
    if (!valid) {
        return res.status(StatusCode_1.default.BAD_REQUEST).json(`O campo ${property} não pode ser nulo ou vazio`);
    }
    return next();
}
;
